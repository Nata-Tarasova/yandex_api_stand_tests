...
# Вспомогательная функция для подготовки тела запроса
def get_user_body(first_name):
    current_body = data.user_body.copy()
    current_body["firstName"] = first_name
    return current_body

# Универсальная функция для позитивных проверок
def positive_assert(first_name):
    ...
    # Формируем строку для поиска в БД
    str_user = user_body["firstName"] + "," + user_body["phone"] + "," \
               + user_body["address"] + ",,," + user_response.json()["authToken"]

    assert users_table_response.text.count(str_user) == 1

# Тест 1. Позитивная проверка: 2 символа в имени
def test_create_user_2_letter_in_first_name_get_success_response():
    positive_assert("Aa")
# Тест 2. Позитивная проверка: 15 символов в имени
def test_create_user_15_letter_in_first_name_get_success_response():
    positive_assert("Ааааааааааааааа")
# Функция для негативной проверки, когда имя введено некорректно
...
# Функция для негативной проверки, когда параметр не передан или передан пустой
def negative_assert_no_first_name(user_body):
    ...
    # Проверка, что в теле ответа атрибут code — 400
    assert response.json()["code"] == 400

    # Проверка, что в теле ответа атрибут message такой:
...
def test_create_user_number_type_first_name_get_error_response():
    # В переменную user_body сохраняется обновлённое тело запроса, где имя — число 12
